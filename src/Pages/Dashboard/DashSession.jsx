// // // DYNAMIC CODE --> with Query Validation Mechanism & Reason
...
// //     const [validationResult, setValidationResult] = useState(null); // Stores validation response (valid/invalid)
...
// //                 // Fetch validation response (valid/invalid)
// //                 const validationResponse = data.valid_answer ? "Answer is valid." : "Answer is invalid.";
...
// //                         {/* Validation Response */}
// //                         {validationResult && (
// //                             <div className={`px-4 py-2 rounded-badge w-fit max-w-[80%] self-start ${validationResult.includes("invalid") ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}>
// //                                 {validationResult}
// //                             </div>
// //                         )}
// //                     </div>
...
// import { useState, useEffect } from 'react';
// import DashMenu from '../../Components/DashMenu';
// import { Plus, MoveRight, ChevronDown, Check, Ellipsis, Info } from 'lucide-react';
// import '../../Components/Custom/CustomScroll.css';
// import { DatabaseZap } from 'lucide-react';
// import API_ENDPOINTS from '../../../config';
...
//     const [loadingMessage, setLoadingMessage] = useState(false); // Track loading state for message
//     const [validationResult, setValidationResult] = useState(null); // Stores validation response (valid/invalid)
//     const [validationReason, setValidationReason] = useState(""); // Stores reason for validation
//     const [isModalOpen, setIsModalOpen] = useState(false); // Controls modal visibility for the reason
...
//                 // Fetch validation response (valid/invalid)
//                 const validationResponse = data.valid_answer ? "Valid" : "Invalid";
//                 setValidationResult(validationResponse);
//                 setValidationReason(data.reason || "No reason provided.");
...
//                         {/* Validation Response */}
//                         {validationResult && (
//                             <div className={`px-4 py-2 rounded-badge w-fit max-w-[80%] self-start ${validationResult === "Invalid" ? "bg-red-500 text-white" : "bg-green-500 text-white"}`}>
//                                 {validationResult}
...
//             {/* Modal for displaying validation reason */}
//             {isModalOpen && (
//                 <div className="modal">
//                     <div className="modal-content">
//                         <h3>Validation Reason</h3>
...
import { useState, useEffect } from 'react';
import DashMenu from '../../Components/DashMenu';
import { Plus, MoveRight, ChevronDown, Check, Ellipsis, Info, Bot, Brain } from 'lucide-react';
import '../../Components/Custom/CustomScroll.css';
import { DatabaseZap } from 'lucide-react';
import API_ENDPOINTS from '../../../config';
                    ...
                    <div className="w-full border rounded-badge p-6 flex flex-col gap-3 transition-transform bg-white">
                        ...
                        <div className="flex flex-col gap-2 overflow-y-auto max-h-48 md:max-h-64 lg:max-h-72 pr-2 custom-scrollbar">
                            {messages.map((msg, index) => (
                                <div
                                    ...
                                    className={`px-4 py-2 rounded-badge w-fit max-w-[80%] ${msg.sender === "user" ? "bg-green-500 text-white self-end" : "bg-gray-300 text-black self-start"}`}
                                >
                                    {/* Show Brain icon only with bot's response */}
                                    {msg.sender === "bot" ? <span><Brain className="inline-block" /> {msg.text}</span> : msg.text}
                                </div>
                            ))}
                        ...
                        {validationResult && (
                            <div className={`px-4 py-2 rounded-badge w-fit max-w-[80%] self-start ${validationResult === "Invalid" ? "bg-red-500 text-white" : "bg-green-500 text-white"}`}>
                                ...
                                {/* Display the validation reason
                                {validationReason && (
                                    <p className="mt-2 text-sm">
                                        <strong>Reason:</strong> {validationReason}
                                    </p>
                                )} */}

                                {/* Show info button for both valid and invalid responses */}
                                <button
                                    onClick={toggleModal}
                                    className="ml-2 text-white hover:text-black"
                                >
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white p-6 rounded-lg max-w-sm w-full shadow-lg">
                        <h3 className="text-xl font-semibold mb-4 flex justify-start items-center gap-2"><span><Bot className='w-6 h-6 text-blue-500' /></span>Validation Reason</h3>
                        <p className="mb-4">{validationReason}</p>
                        <button
                            onClick={toggleModal}
                            className="bg-black text-white px-4 py-2 rounded-full hover:bg-green-600 focus:outline-none"
                        >
                            Close
                        </button>
                    </div>
